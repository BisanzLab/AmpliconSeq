---
title: "Bisanz Lab QIIME2 Pipeline v2.0: qiime2-2023.5"
...
```{bash settings}
echo "export TemplateXLSX=/data/SequencingRuns/2023May9_IvHg_BAscreen_CBD/library_construction/230504_DBA_MSK_AmpliconSeq_TrackingSheet.xlsx" > settings.txt #an absolute link to the location of your run template
echo "export Project=*" >> settings.txt # This would be the Sample_Project Column. Leave this as * if you want to get all samples from the
echo "export ReadDir=/data/SequencingRuns/2023May9_IvHg_BAscreen_CBD/reads/" >> settings.txt #an absolute directory containing your demultiplexed reads
...
SampleSheet<-SampleSheet %>% left_join(fastqs) %>% filter(grepl(Project, Sample_Project))
...
ReadTracking %>%
  left_join(
    read_excel(TemplateXLSX, sheet="Loading.csv", skip=1) %>% dplyr::rename(Sample_Plate=plate, Sample_Well=well, Volume_ul=volume)
  )
...
Note: samples with less than 5,000 reads are being filtered out before downstream analysis.
